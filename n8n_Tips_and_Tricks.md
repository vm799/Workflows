...
Integration & Memory:  
• Connects with subfunctions via ports like \`ai\_tool\`, \`ai\_memory\`, and \`ai\_languageModel\`.  
...
Key Parameters:  
• webhookId (for triggers), chatId, text, file (URL), and additionalFields (e.g., parse\_mode).  
Nuances:  
