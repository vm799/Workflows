...
3\. \*\*Critical Configuration Requirements\*\*    
   ...
   \- Each \*\*OpenAI\*\* node that returns structured data must have \`"responseFormat": "json\_object"\`.  
...
4\. \*\*No Placeholders or Partial JSON\*\*    
   \- Do not provide placeholders like \`"API\_KEY\_HERE"\` or \`\[YOUR DOC ID\]\`. Use a generic reference if needed (e.g., \`"{{ myCredentials }}"\`).  
